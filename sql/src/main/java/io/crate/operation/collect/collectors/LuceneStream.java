...
public class LuceneStream {
    ...
    public static Stream<Row> createStream(IndexSearcher searcher,
                                           Query query,
                                           boolean doScores,
                                           BiFunction<LeafReaderContext, Scorer, Stream<Row>> flatMapper) throws IOException {
        Weight weight = searcher.createNormalizedWeight(query, doScores);
        return searcher.getTopReaderContext().leaves().stream()
            .flatMap(leafReaderContext -> {
                Scorer scorer;
                try {
                    scorer = weight.scorer(leafReaderContext);
                } catch (IOException e) {
                ...
                return flatMapper.apply(leafReaderContext, scorer);
            });
    }
    ...
    public static Stream<Row> stream(IndexSearcher searcher,
        ...
        return createStream(
            searcher,
            query,
            doScores || minScore != null,
            (readerContext, scorer) -> {
                for (LuceneCollectorExpression<?> expression : expressions) {
                    try {
